// The program converts a prefix expression into its corresponding infix expression
#include <iostream>
#include <string>
#include <cmath>
#include <cctype>
...
bool validatePrefix(string);                                // Function to validate the prefix expression
void parsePrefix(string, vector<string> &);                 // Function to parse prefix into operands and operators
string prefixToInfix(vector<string>);                       // Function to convert from prefix to infix expression
...
bool areAllSymbolsValid(string);                            // Function to check if all characters in the prefix are valid
...
int main()
{
    ...
    string prefixExpression,        // String to hold the prefix expression from user
           infixExpression,         // String to hold the infix expression
           userChoice;              // String to hold user's choice
    int choice;                     // Variable to store user's choice
    vector<string> prefixTokens,    // Vector to store the prefix tokens
    ...
    do
    {
        ...
        // If the user's choice is OPTION_1: Enter a prefix expression
        if (choice == OPTION_1)
        {
            ...
            // If the prefix is valid, then display its infix
            if (validatePrefix(prefixExpression))
            {
                // Parsing the prefix into operands and operators
                parsePrefix(prefixExpression, prefixTokens);

                // Convert the prefix into infix
                infixExpression = prefixToInfix(prefixTokens);

                // Display the infix
                cout << "Infix: " << infixExpression << endl;
            }
            // Otherwise, if the prefix is invalid, display error message
            else
            {
                cout << "Invalid prefix.\n";
            }
...
{
    cout << "------ WELCOME TO PREFIX-TO-INFIX CONVERTER ------\n\n";
    cout << "This program converts a mathematical expression written in prefix notation ";
    cout << "(operators before operands) into its equivalent infix notation ";
    cout << "(operators between operands with parentheses for clarity).\n";
    cout << "For example, the prefix notation of 3 + 5 is + 3 5.\n";
    cout << "\nConventions:\n";
    cout << "1. Difference operands should be separated by spaces to avoid ambiguity. For example, + 355 can be interpreted as + 3 55 or + 35 5, so you should add spaces to clarify you prefix expression.\n";
...
// **************************************************
// Definition of function validatePrefix            *
// This function validates a prefix expression. It  *
// returns false if the prefix is invalid, and true *
...
bool validatePrefix(string prefix)
{
    ...
    // Prefix expression must contain only operators and digits
    if (!areAllSymbolsValid(prefix))
    {
        cout << "ERROR: Invalid characters in prefix\n";
        return false;
    }

    // Parsing the prefix into operands and operators
    parsePrefix(prefix, prefixTokens);

    // Count the number of operators and operands
    numOperators = countSymbol(prefixTokens, OPERATOR);
    numOperands = countSymbol(prefixTokens, NUMBERS);

    if (numOperands == 0 && numOperators == 0)
    {
        cout << "ERROR: Empty prefix.\n";
        return false;
    }

    if (numOperands == 0)
    {
        cout << "ERROR: No any operands in the prefix.\n";
        return false;
    }

    if (numOperators == 0)
    {
        cout << "ERROR: No any operators in the prefix.\n";
        return false;
    }
    ...
    // There must be at least two operands following an operator at the end
    const int OPERANDS_PER_OPERATOR = 2;
    int length = prefixTokens.size();
    numOperands = 0;

    for (int index = length - 1; index >= 0; index--)
    {
        if (isOperator(prefixTokens[index]))
        {
            index++;
            while (index < length && isOperand(prefixTokens[index]))
            {
                numOperands++;
                index++;
            }
    ...
    {
        cout << "ERROR: Lack of operands at the end.\n";
        return false;
    }

    // Operators must be in front of operands
    if (isOperator(prefixTokens[prefixTokens.size() - 1]))
    {
...
// **************************************************
// Definition of function parsePrefix               *
// This function will parse the prefix into its     *
// operands and operators only, and put them into   *
// a vector.                                        *
// **************************************************
void parsePrefix(string prefix, vector<string> &prefixTokens)
{
    int count = 0;      // Loop counter variable
    string token;

    // Read the prefix from its left to right
    while (prefix[count] != '\0')
    {
        ...
        if (isdigit(prefix[count]))
        {
            ...
            count--;
            // Push the token into the prefixToken vector
            prefixTokens.push_back(token);
        }
        // If the character is a minus and at the start of the prefix
        else if (count == 0 && prefix[count] == '-')
        {
            ...
            count--;
            // Push the token back into the prefixToken vector
            prefixTokens.push_back(token);
        }
        // Else if the character is a minus, preceded by a space, and in the middle
        else if (count > 0 && prefix[count] == '-' && isspace(prefix[count - 1]))
        {
            // Check for a multiple-digit negative number
            count++;
            // While the next character is not null and is a digit
...
// **************************************************
// Definition of function prefixToInfix             *
// This function converts a prefix into an infix,   *
// and returns the string of infix.                 *
// **************************************************
string prefixToInfix(vector<string> prefixTokens)
{
    ...
    // Scanning the prefixTokens vector from the right to left
    for (int index = prefixTokens.size() - 1; index >= 0; index--)
    {
        token = prefixTokens[index];
        ...
        else if (isOperator(token))
        {
            ...
            // Get the associativity of the precedence
...
// **************************************************
int countSymbol(vector<string> prefixTokens, char symbol)
{
    int count = 0;
...
// The function checks if a prefix contains only    *
// valid characters or not.                         *
// **************************************************
bool areAllSymbolsValid(string prefix)
{
    char character;
