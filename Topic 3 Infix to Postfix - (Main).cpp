// The program converts an infix notation into postfix notation
#include <iostream>
#include <vector>
#include <string>
#include <cmath>
...
void infixToPostfix(const vector<string>, vector<string> &); // Function converts infix to postfix
double postfixCalculation(const vector<string>);              // Function to evaluate postfix
...
int main()
{
    ...
    vector<string> postfixTokens;                           // Vector to hold the tokens of prefix notatio
    ...
    do
    {
        ...
        // If the user's option is OPTION_2: Calculate the postfix
        else if (choice == OPTION_2)
        {
            // If there is no postfix expression
            if (postfixTokens.empty())
            {
                cout << "There is no postfix expression to calculate.\n"
                     << "You should type in an infix expression.\n";
            }
            // If there is a postfix expression, calculate and display result
            else
            {
                double result = postfixCalculation(postfixTokens);
...
{
    cout << "------ WELCOME TO INFIX-TO-POSTFIX CONVERSION ------\n\n";
    cout << "This program converts mathematical expressions from infix notation to postfix notation.\n\n";
    cout << "Guidelines:\n";
...
void displayMenu()
{
    ...
    cout << "2. Calculate the postfix expression\n";
    cout << "3. Exit\n\n";
...
// **************************************************
// Definition of function infixToPostfix            *
// This function converts infix to postfix          *
// **************************************************
void infixToPostfix(const vector<string> infixTokens, vector<string> &postfixTokens)
{
    ...
    {
        // If the token is an operand, add it to postfixTokens
		if (isOperand(token))
        {
        ...
        {
			// If the stack is not empty, then check the precedence of the operator
            ...
            {
                // Pop the last operator in opStack while the incoming operator is larger than the top operator
				...
				// Pop the last operator in opStack while the incoming operator
				// is equal to the top operator and it is left associativity
				while (!opStack.empty() && precedence(token) == precedence(opStack.back()))
                {
                    // Take the associativity rule
                    char associativity = takeAssociativity(token);

                    // If the associativity is from left to right
                    if (associativity == 'L')
                    {
                        // Push the top operator to postfixTokens
                        postfixTokens.push_back(opStack.back());
...
// **************************************************
// Definition of function postfixCalculation        *
// This function evaluates the postfix expression   *
// **************************************************
double postfixCalculation(const vector<string> postfixTokens)
{
    vector<double> operands;
    double result;

    // Scanning the postfix expression from left to right
    for (int index = 0; index < postfixTokens.size(); index++)
    {
        string token = postfixTokens[index];

        // If the token is an operand
        if (!token.empty() && isOperand(token))
        {
            // Convert a string of numeric value into a number
            int numbers = stoi(token);
            // Push the number back into the operands vector
            operands.push_back(numbers);
        }
        // If the token is an operator
        else
        {
            // Take the second operand in the operands vector
            double operand_2 = operands.back();
            operands.pop_back();

            // Take the first operand in the operands vector
